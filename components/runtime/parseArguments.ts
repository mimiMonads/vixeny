...
export default (): ParsedArgs =>
  ...
  (typeof Deno !== "undefined" ? Deno.args : process.argv.slice(2))
    .map<[string, string | boolean]>((arg: string) => arg.startsWith('--') ? arg.slice(2).split("=") as [string, string] : [arg, true])
    .reduce<ParsedArgs>((acc: ParsedArgs, [key, value]: [string, string | boolean]) => ({
      ...acc,
