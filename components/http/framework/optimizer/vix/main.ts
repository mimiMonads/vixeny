import { FunRouterOptions, Petitions } from "../../types.ts";
import { SignVerifyOptions } from "../../components/tokens/types.ts";
import vixeny from "../../fun.ts";
import mapping from "./map.ts";
type CustomVerifyVixeny = FunRouterOptions & {
  at: string;
  401?: { (r: Request): boolean } | { async(r: Request): boolean };
  customVerify: { (r: Request): boolean } | { async(r: Request): boolean };
  sideEffectOnAccept?: { (r: Request): boolean } | {
    async(r: Request): boolean;
  };
  sideEffectOnReject?: { (r: Request): boolean } | {
    async(r: Request): boolean;
  };
};

type VixVixeny = FunRouterOptions & {
  at: string;
  401?: { (r: Request): boolean } | { async(r: Request): boolean };
