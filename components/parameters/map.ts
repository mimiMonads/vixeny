            ...
            (endsInSlash) =>
              (
                (point) => ({
                  ...
                  lastParam:
                    list.slice(point.head + 1, list.length).reduce((acc, x) =>
                      x.length + 1 + acc, 0) + (endsInSlash ? 1 : 0),
                  endsInSlash: endsInSlash,
                  hasName: options?.hasName,
                })
              )({
                tail: list.reduce((acc, x, i) =>
                  (x[0] === startsWith && acc === -1) ? i : acc, -1),
                head: list.reduceRight((acc, x, i) =>
                  (x[0] === startsWith && acc === 0) ? i : acc, 0),
              })
          )(
            f.path.at(-1) === "/",
          ))(
