...
export default (o?: FunRouterOptions<any>) => (path: string) => async (input: ResolveMap<any>): Promise<ResponseResponse> => {
  // Convert input ResolveMap to ResolveOptions array
  const ar = Object.keys(input).map((x) => ({ ...input[x], name: x })) as ResolveOptions;

  // Await the table function since it might now be async
  const tableResult = await table(o)(path)(ar) as { name: string; f: (r: any) => any | Promise<any> }[];

  // Check if any function in the tableResult is asynchronous
  const isAsync = tableResult.some((x) => x.f.constructor.name === "AsyncFunction");
